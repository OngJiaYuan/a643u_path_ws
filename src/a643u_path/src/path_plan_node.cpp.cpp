...
std::vector<std::array<int, 2>> findPath(int idx_x, int idx_y, int goal_x, int goal_y)
{
  ...
  while (!queue.empty())
  {
    ...
    Node *cur_node = queue.front(); //  The position of the vertex in the front of the queue
    queue.pop_front();              //  remove vertex and mark as explored 
    int cur_x = cur_node->x;        
    int cur_y = cur_node->y;
    int cur_cost = cur_node->cost;

    // ---------------------- (2) Find the path if goal is found, and break loop -------------------------
    if (cur_x == goal_x && cur_y == goal_y)   // if goal == current vertex execute break loop function 
    {
      Node *node = cur_node;
      do
      {
